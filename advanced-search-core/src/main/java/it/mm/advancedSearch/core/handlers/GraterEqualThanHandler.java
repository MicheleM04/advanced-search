...
public class GraterEqualThanHandler implements AdvancedSearchOperatorHandler {
	...
	public void handle(List<Predicate> predicates, CriteriaBuilder cb, Expression<?> path, Object value) {
		if (value != null) {
			if (value instanceof Date) {
				...
				predicates.add(cb.greaterThanOrEqualTo((Expression<Date>) path, dateValue));
				...
				predicates.add(cb.ge((Expression<Number>) path, numberValue));
			} else {
				invalidType();
			}
		}
